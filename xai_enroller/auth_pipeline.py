...
class AuthPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self._clock = clock
        self.oauth_mode = str(oauth_mode or "auth_code").strip().lower()
        self.proxy = proxy or ""
        self._auth_code_credentials = {}
    ...
    async def _start_flow(self):
        if self.oauth_mode == "auth_code":
            # Placeholder only; real convert happens in _authorize via Auth Code + PKCE.
            return DeviceFlow(
                device_code="auth_code",
                user_code="AUTHCODE",
                verification_url="https://auth.x.ai/oauth2/authorize",
                expires_in=3600,
                interval=1.0,
    ...
    async def _authorize(self, prepared):
        ...
        try:
            ...
            if self.oauth_mode == "auth_code":
                ...
                try:
                    ...
                    credential = OAuthCredential(
                        ...
                        expires_in=int(entry.get("expires_in") or 21600),
                        expires_at=str(entry.get("expired") or ""),
                        last_refresh=str(entry.get("last_refresh") or ""),
                        subject=entry.get("sub"),
                        token_endpoint=str(entry.get("token_endpoint") or "https://auth.x.ai/oauth2/token"),
                        email=entry.get("email") or prepared.source.source_id,
                        base_url=str(entry.get("base_url") or "https://cli-chat-proxy.grok.com/v1"),
                        redirect_uri=entry.get("redirect_uri"),
                        headers=entry.get("headers") if isinstance(entry.get("headers"), dict) else None,
                    )
                    self._auth_code_credentials[prepared.job_id] = credential
                    await self.rate_gate.authorized(probe_token)
                    await self.completion_queue.put(CompletionJob(prepared))
                    return
                except ConvertError as error:
                    reason = str(error)
                    status = JobStatus.OAUTH_REJECTED
                    low = reason.lower()
            ...
            if authorization.status is AuthorizationStatus.AUTHORIZED:
                # Browser UI can show a false "done" page after the OAuth
                # flow changed. Probe the token endpoint once before treating
                # confirmation as real.
                probe = await self._probe_grant_state(prepared)
                if probe == "rejected":
                    await self.rate_gate.inconclusive(probe_token)
                    await self._finish_failure(
                        prepared.source,
                        ...
                        retry=prepared.attempt_number < 3,
                        task_number=prepared.task_number,
                    )
                    return
    ...
    async def _probe_grant_state(self, prepared):
        """Return pending|ready|rejected|unknown for the device grant."""
        try:
            response = await self.protocol.client.post(
                prepared.flow.token_endpoint,
                data={
                    "client_id": self.protocol.profile.client_id,
                    "device_code": prepared.flow.device_code,
                    "grant_type": "urn:ietf:params:oauth:grant-type:device_code",
                },
                follow_redirects=False,
            )
            document = await self.protocol._json(response)
        except Exception:
            return "unknown"
        if response.status_code // 100 == 2:
            if document.get("access_token") and document.get("refresh_token"):
                return "ready"
            return "rejected"
        error = document.get("error")
        if error in {"authorization_pending", "slow_down"}:
            return "pending"
        if error in {"invalid_grant", "access_denied", "expired_token"}:
            return "rejected"
        return "unknown"
    ...
    async def _complete(self, completion):
        ...
        try:
            if self.oauth_mode == "auth_code" and prepared.job_id in self._auth_code_credentials:
                credential = self._auth_code_credentials.pop(prepared.job_id)
            else:
                async with asyncio.timeout(self.timeout):
            ...
            self._completion_cancellable = False
            reason = str(error)
            if reason in {"oauth_denied", "oauth_expired"} or reason == "oauth_rejected" or reason.startswith("oauth_rejected:"):
                status = {
                    "oauth_denied": JobStatus.OAUTH_DENIED,
                    "oauth_expired": JobStatus.OAUTH_EXPIRED,
                }.get(reason, JobStatus.OAUTH_REJECTED)
                # invalid_grant after a "confirmed" page is usually incomplete
                # device binding / false success. Retry a few times instead of
                # permanently burning the SSO session on first false positive.
                retry = False
                if reason.endswith("invalid_grant") and prepared.attempt_number < 3:
                    retry = True
                await self._finish_failure(
                    prepared.source,
                    prepared.source_fingerprint,
