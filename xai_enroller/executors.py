        ...
        async with self._semaphore:
            ...
            try:
                ...
                while asyncio.get_running_loop().time() < deadline:
                    ...
                    done_page = "/oauth2/device/done" in parsed.path or any(
                        marker in text_lower
                        for marker in (
                            "device authorized",
                            "设备已授权",
                    ...
                    if done_page:
                        # Require real interaction evidence. A bare "done" page
                        # without submit/consent often means UI noise and leads
                        # to token invalid_grant.
                        if code_submitted or consent_submitted or verify_succeeded:
                    ...
                    if "/oauth2/device/consent" in parsed.path or any(
                        ...
                        if not consent_submitted:
                            allowed = await self._click_visible_exact(
                                page,
                                ("允许", "Allow", "Authorize", "Approve", "继续", "Continue"),
                            )
                            if allowed:
                    ...
                    if code_input_count and await code_input.first.is_visible():
                        ...
                        if not submit_count:
                            # New Grok Build page may use a non-submit button labeled 继续/Continue.
                            clicked = await self._click_visible_exact(
                                page,
                                ("继续", "Continue", "Confirm", "确认"),
                            )
                            if not clicked:
                                return AuthorizationResult(
                                    AuthorizationStatus.NEEDS_INTERACTION,
                        ...
                        predicate = lambda response: (
                            ...
                            in {"auth.x.ai", "accounts.x.ai"}
                            and (
                                urlparse(response.url).path == "/oauth2/device/verify"
                                or "/oauth2/device" in urlparse(response.url).path
                            )
                        )
                        try:
                            ...
                            # Do not mark success evidence without a verify response.
                            await submit.first.click()
                            unknown_since = None
                            await page.wait_for_timeout(800)
                            continue
                    ...
                    # Clicking Continue/Authorize only advances the flow.
                    # Success must be proven by /oauth2/device/done or
                    # "device authorized" text above, never by a single click.
                    if await self._click_visible_exact(
                        ...
                        unknown_since = None
                        # Mark consent only on explicit consent page.
                        if "/oauth2/device/consent" in parsed.path:
                            consent_submitted = True
                        # If this click triggered device verify, record evidence.
                        try:
                            response = await page.wait_for_response(
                                lambda resp: (
                                    urlparse(resp.url).hostname
                                    in {"auth.x.ai", "accounts.x.ai"}
                                    and "/oauth2/device" in urlparse(resp.url).path
                                ),
                                timeout=3000,
                            )
                            location = urlparse(response.headers.get("location", ""))
