...
class EventTerminal:
    ...
    def _format_debug(self, kind, data):
        ...
        if kind == "result":
            ...
            reason = data.get("reason")
            if isinstance(reason, str) and reason.startswith("oauth_rejected:"):
                detail = reason.split(":", 1)[1]
                safe_detail = "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in detail)[:64]
                safe_reason = f"oauth_rejected:{safe_detail}" if safe_detail else "oauth_rejected"
            else:
...
async def main_async(*, log_mode="user"):
    ...
    from .sinks import CPAAuthFileSink, LocalAuthFileSink, MultiSink
    ...
    try:
        ...
        local_sink = LocalAuthFileSink(
            Path(settings.local_auth_dir) / AUTHENTICATED_DIRNAME,
            name_secret=settings.source_salt,
        )
        # Prefer dual-write when CPA is configured: local file + CPA auth-files.
        cpa_base = (
            os.environ.get("XAI_ENROLLER_CPA_BASE_URL")
            or os.environ.get("CPA_BASE_URL")
            or os.environ.get("CPA_BASE")
            or ""
        ).strip()
        cpa_secret = (
            os.environ.get("XAI_ENROLLER_CPA_MANAGEMENT_SECRET")
            or os.environ.get("CPA_MANAGEMENT_SECRET")
            or os.environ.get("CPA_KEY")
            or os.environ.get("MANAGEMENT_KEY")
            or ""
        ).strip()
        if cpa_base and cpa_secret:
            cpa_sink = CPAAuthFileSink(
                cpa_base,
                cpa_secret,
                client,
                name_secret=settings.source_salt,
            )
            sink = MultiSink([local_sink, cpa_sink])
        else:
            sink = local_sink
        ...
        terminal.emit(
            (
                "startup",
                {
                    ...
                    "oauth_mode": (
                        os.environ.get("XAI_ENROLLER_OAUTH_MODE")
                        or os.environ.get("OAUTH_MODE")
                        or "auth_code"
                    ).strip().lower(),
                    "cpa_upload": bool(cpa_base and cpa_secret),
        ...
        proxy = (
            os.environ.get("XAI_ENROLLER_PROXY")
        ...
        if not proxy and os.environ.get("XAI_ENROLLER_PROXY_DISABLE", "").strip().lower() not in (
            "1",
            "true",
            "yes",
            "on",
        ):
            # Best-effort Clash probe (Windows host from WSL = resolv nameserver).
            try:
                import socket
                ...
                for url in cands:
                    u = urlparse(url)
                    try:
                        with socket.create_connection((u.hostname, int(u.port or 0)), timeout=0.35):
                            proxy = url
                            break
                    except Exception:
                        continue
