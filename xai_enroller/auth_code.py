#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""SSO → OAuth Authorization Code + PKCE (Grok Build).

Ported from upstream UI flow register-win/lib/sso2cpa_core.py.

This is the working free-Grok path:
  SSO cookie → /oauth2/authorize (referrer=grok-build) → consent allow
  → authorization_code → access/refresh with JWT claim referrer=grok-build
  → CPA/Sub2 compatible auth JSON (base_url=cli-chat-proxy.grok.com/v1)

Device-flow is intentionally NOT used here.
"""
    ...
    if not code:
        ...
        if "error=access_denied" in (redirect_url or "") or "access_denied" in (
            ...
            raise ConvertError(
                ...
                "常见原因：1) 注册时缺 Castle token（账号 botFlag=CASTLE，旧号通常无法修复，需重注册）；"
                "2) 转换未走代理；3) SSO 失效。可查 grok.com/rest/auth/get-user 是否含 botFlagSource；"
...
def convert_to_oauth_credential(sso: str, email: str = "", proxy: str = ""):
    """Return (cpa_entry_dict, OAuthCredential-like fields) for local sinks."""
    entry = convert_one(sso, email=email, proxy=proxy)
    return entry


def require_grok_build(access_token: str) -> str:
    claims = decode_jwt_payload(access_token or "")
    ref = str(claims.get("referrer") or "").strip()
    if ref != GROK_REFERRER:
        raise ConvertError(
            f"access_token referrer={ref!r}（必须 {GROK_REFERRER!r}）"
