...
class XAIProtocol:
    ...
    @staticmethod
    def _safe_oauth_error(error):
        if not isinstance(error, str):
            return None
        cleaned = "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in error.strip().lower())
    ...
    async def poll_token(self, *, endpoint, flow, timeout):
        ...
        while time.monotonic() < deadline:
            ...
            safe = self._safe_oauth_error(error)
            if safe:
                raise RuntimeError(f"oauth_rejected:{safe}")
