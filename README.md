...
1. 本工具仅供**学习、研究与个人自用**的自动化技术交流，请遵守 xAI / Grok 服务条款与当地法律法规。
...
| OAuth | 偏 Device Flow / 认证服务导出 | 默认 **Authorization Code + PKCE**（`referrer=grok-build`），降低常见 `invalid_grant` |
| 产物 | SSO / 认证服务 claimed 文件 | 生成 **CPA 兼容** `auth-local/authenticated/xai-*.json`，可选上传 CLIProxyAPI |
...
> 二开目标是「注册 → 授权 → 可导入 CPA / 同类网关」闭环；原版 CSP 注册链路仍保留，可按环境选用。

## 项目优势

- **注册 + 授权一体**：从 SSO 会话到 OAuth `access_token` / `refresh_token`
...
- **台账跳过**：`keys/cpa-enroll-ledger.jsonl` 记住成功项，重复跑可跳过已成功账号

## 限制与已知问题

- 依赖第三方页面与风控（Turnstile / Castle 等），成功率随 IP、指纹、邮箱域名变化
- 部分邮箱域名会被判定为临时邮，OAuth 可能出现 `access_denied`（需换更可信域名）
- Device Flow 路径仍保留作兼容 / 实验，**默认不推荐**（易 `invalid_grant`）
- 代理与出口节点请自行管理；仓库不捆绑代理配置
...
# 跳过日志静默：不逐条输出 [SKIP]（仍保留 skip 计数），大量已授权账号时减少刷屏
.\.venv\Scripts\python.exe -u .\授权.py --quiet-skip
```
...
**Q: 可以不上传 CPA，只留本地文件吗？**  
A: 可以。本地会写入 `auth-local/authenticated/`；上传需配置 CPA 并使用 `--upload-cpa`。
