...
class FakePage:
    def __init__(self):
        self.closed = False
        self.url = "https://accounts.x.ai/oauth2/device"
        self._text = "Authorize access"
    ...
    def get_by_role(self, role, name, exact=False):
        if self._authorized:
            return FakeButton(0)
        if name in {"Authorize", "Allow", "Continue", "继续"}:
            page = self

            class AuthorizingButton(FakeButton):
                async def click(self):
                    page._authorized = True
                    page._text = "Device authorized"
                    page.url = "https://accounts.x.ai/oauth2/device/done"

            return AuthorizingButton(1)
        return FakeButton(0)
